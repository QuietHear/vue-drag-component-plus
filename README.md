...
>
	尽量不要动态改变，不然之前组合的宽度会有问题，改变后需要调用changeGroupBorder()方法重新计算组件大小
>
...
* `itemMinWidth`：组件项最小宽度，实际值会叠加纵向间距，校验变量不能小于15-->Number;非必传;默认*15*

* `itemMinHeight`：组件项最小高度，实际值会叠加横向间距，校验变量不能小于15-->Number;非必传;默认*15*
