...
* `changeGroupBorder`：组合边框大小改变重新计算组件大小（需要手动调用）
...
* `@dragStart`：开始拖拽-->第一个当前操作对象

* `@dragIng`：正在拖拽-->第一个当前操作对象

* `@dragEnd`：结束拖拽-->第一个当前操作对象

* `@resizeStart`：开始收缩-->第一个当前操作对象

* `@resizeIng`：正在收缩-->第一个当前操作对象

* `@resizeEnd`：结束收缩-->第一个当前操作对象

* `@openSetMenu`：打开设置菜单-->第一个当前操作对象
