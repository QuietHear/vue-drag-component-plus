...
* `adsorbSpace`：拖拽结束时、、缩放过程中自动吸附的最大间距，校验变量大于等于0-->Number;非必传;默认*10*
>
	1. 小于等于设置值时会进行吸附
	2. 设置为0的话则会关闭吸附效果
	3. 辅助线纵向的对齐（先左再中后右），三种情况只会生效一种
>
...
* `@hasError`：执行方法过程中发生的错误-->第一个参数返回错误信息
>
	可以自己加错误提示等逻辑；如果是提示数值格式转换异常，建议提示保存画布数据并刷新页面，然后使用自动修复或自行排查异常数据
>

* `@dealError`：执行init()后如果存在位置信息丢失的异常数据会自动修复并抛出原数据-->第一个参数返回存在异常的数据集合
>
	可以自己加错误提示等逻辑
>
